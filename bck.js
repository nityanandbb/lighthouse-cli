// Delete revert this ref file after QA teams successfull reporting.
...
const generateFinalHTMLReport = (summaryData, configData, analysisData) => {
  ...
  sortedUrls.forEach((url) => {
    ...
    console.log(desktopAverage + "d");
    siteMobileAvgPerf.push(mobileAverage);
    console.log(mobileAverage + "m");
    siteDesktopSEO.push(desktopSEO);
    siteMobileSEO.push(mobileSEO);

    // Step 3: Calculate total site average performance
    function calculateTotalSiteAvgPerformance() {
      if (siteDesktopAvgPerf.length === 0) return 0;

      const total = siteDesktopAvgPerf.reduce((acc, curr) => acc + curr, 0);
      const avg = total / siteDesktopAvgPerf.length;
      return avg;
    }

    // Final result
    const totalPerformance = calculateTotalSiteAvgPerformance();
    console.log(`Total Site Avg Performance: ${totalPerformance}`);
    ...
    const siteTotalAvgFromAvgTotal = (itsFor, avgOfAvg) => {
      ...
      const avg = avgOfAvg.length > 0 ? total / avgOfAvg.length : 0;
      return avg;
    };

    let avgPerformanceDesktop = siteTotalAvgFromAvgTotal(
      "desktop",
      siteDesktopAvgPerf
    );
    // console.log("avg perf desktop " + avgperformanceDesktop);

    let avgPerformanceMobile = siteTotalAvgFromAvgTotal(
      "mobile",
      siteMobileAvgPerf
    );
    ...
    // Calculations :-
    // individual lcp
    const avgLCP = calculateMobileAverageAudit("largestContentfulPaint");
    const avgFCP = calculateMobileAverageAudit("firstContentfulPaint");
    const avgTBT = calculateMobileAverageAudit("totalBlockingTime");
    const avgCLS = 0.1;
    const avgSpeedIndex = calculateMobileAverageAudit("speedIndex");

    let avgAllSite_DesktopSEO = siteTotalAvgFromAvgTotal(
      "desktop",
      siteDesktopSEO
    );
    let avgAllSite_MobileSEO = siteTotalAvgFromAvgTotal(
      "mobile",
      siteMobileSEO
    );

    // All urls lcp avg
    siteAvgLCP.push[avgLCP];
    let avgAllSite_DesktopLCP = siteTotalAvgFromAvgTotal("mobile", siteAvgLCP);

    console.log(
      "mobile, desktop " + avgMobileSEO * 100 + " _  " + avgDesktopSEO * 100
    );

    // upto this line :- need to debug and update above.
...
console.log("Enhanced Lighthouse report generation completed!");
