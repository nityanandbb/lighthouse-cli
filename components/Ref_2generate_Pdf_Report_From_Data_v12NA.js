...
const generatePdfReportFromData = async (logo, summaryData, configData) => {
    ...
    return Object.entries(grouped)
      .map(([url, types]) => {
        ...
        if (types.desktop) {
          ...
          html += `<tr><td>Desktop</td><td style="word-wrap: break-word; max-width: 300px;">${url}</td><td>${getColoredPercentage(
        ...
        if (types.mobile) {
          ...
          html += `<tr><td>Mobile</td><td style="word-wrap: break-word; max-width: 300px;">${url}</td><td>${getColoredPercentage(
  ...
  const html = `
    <html><head><style>
      ...
      h3 { font-size: 16px; margin-top: 40px; text-decoration: underline; }
      table { width: 100%; border-collapse: collapse; margin-top: 20px; word-wrap: break-word; }
